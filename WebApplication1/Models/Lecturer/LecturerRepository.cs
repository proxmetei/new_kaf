...
namespace OnlineChat.Models.Lecturer
{
        ...
        public bool setQuiz(QuizModel quiz)
        {

            string sqlInsert = "INSERT INTO public.quiz(lecturer_id, " +
                "aspiranture, experience, angular, react, vue, nodejs, workload ) " +
                "VALUES (@LID, @ASP, @EXP, @ANG, @REA,@VUE, @NOD, @WOR)";

            connection.Execute(sqlInsert, new
            {
                LID = quiz.LecturerId,
                ASP = quiz.Aspirantura,
                EXP = quiz.Experience,
                ANG = quiz.Angular,
                REA = quiz.React,
                VUE = quiz.Vue,
        ...
        public List<QuizModel> getAllQuizes() {
            string sql = "SELECT id AS Id, aspiranture AS Aspirantura, " +
                        "experience AS Experience, angular AS Angular, " +
                        "lecturer_id AS LecturerId, react AS React,vue AS Vue, nodejs AS NodeJs, workload as Workload " +
                        "FROM public.quiz";

            return connection.Query<QuizModel>(sql).ToList();
